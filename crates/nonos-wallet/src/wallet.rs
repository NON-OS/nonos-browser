use nonos_crypto::{
    check_stealth_address, derive_blake3_key_from_mnemonic, derive_eth_address_from_private,
    derive_secp256k1_key, derive_stealth_private_key, sign_message, sign_personal_message,
    SecureMnemonic, StealthKeyPair, StealthMetaAddress,
};
use nonos_types::{
    Blake3Hash, Blake3Key, EcdsaSignature, EthAddress, NonosError, NonosResult,
    Secp256k1PrivateKey, Secp256k1PublicKey, TransactionRecord, TransactionStatus, WalletId,
    WalletMetadata,
};
...
impl Wallet {
    ...
    pub fn unlock_with_blake3_key(&mut self, key_hex: &str) -> NonosResult<()> {
        ...
        if address != self.metadata.address {
            return Err(NonosError::Wallet(
                "BLAKE3 key does not match wallet".into(),
            ));
        }
...
mod tests {
    ...
    fn test_wallet_builder() {
        let (wallet, mnemonic, key) = WalletBuilder::new().name("Builder Wallet").build().unwrap();
