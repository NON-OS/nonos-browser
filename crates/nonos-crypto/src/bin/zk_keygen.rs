...
impl ConstraintSynthesizer<Fr> for IdentityCircuit {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fr>) -> Result<(), SynthesisError> {
        ...
        let merkle_path = self
            .merkle_path
            .unwrap_or_else(|| vec![Fr::from(0u64); MERKLE_DEPTH]);
        ...
        let computed_nullifier =
            poseidon_hash3_gadget(cs.clone(), &secret_var, &commitment_var, &scope_var)?;
...
fn generate_identity_keys(output_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn show_info(keys_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
