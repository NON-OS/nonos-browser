...
pub fn encrypt(key: &Blake3Key, plaintext: &[u8]) -> NonosResult<Vec<u8>> {
    let cipher =
        Aes256Gcm::new_from_slice(&key.0).map_err(|e| NonosError::Crypto(e.to_string()))?;
...
pub fn encrypt_with_aad(key: &Blake3Key, plaintext: &[u8], aad: &[u8]) -> NonosResult<Vec<u8>> {
...
pub fn decrypt_with_aad(key: &Blake3Key, encrypted: &[u8], aad: &[u8]) -> NonosResult<Vec<u8>> {
    use aes_gcm::aead::Payload;
