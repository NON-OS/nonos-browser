...
impl StealthMetaAddress {
    ...
    pub fn decode(s: &str) -> NonosResult<Self> {
        let s = s.strip_prefix("st:eth:0x").ok_or_else(|| {
            NonosError::InvalidAddress("Invalid stealth meta-address prefix".into())
        })?;
...
pub fn generate_stealth_address(
    ...
    let shared_secret =
        compute_ecdh_shared_secret(&ephemeral_private, &recipient_meta.view_pubkey)?;
    ...
    let scalar_sk =
        SecretKey::from_slice(&stealth_scalar.0).map_err(|e| NonosError::Crypto(e.to_string()))?;

    let scalar_pk = PublicKey::from_secret_key(&ctx, &scalar_sk);

    let stealth_pk = spend_pk
        .combine(&scalar_pk)
    ...
    let stealth_address = StealthAddress::new(ephemeral_public, view_tag, recipient_meta.hash());
...
pub fn derive_stealth_private_key(
    ...
    let stealth_sk = spend_sk
        .add_tweak(&secp256k1::Scalar::from(scalar_sk))
        .map_err(|e| NonosError::Crypto(e.to_string()))?;
...
mod tests {
    ...
    fn test_stealth_address_generation_and_scanning() {
        ...
        let result =
            scanner.scan_announcement(&stealth_addr.ephemeral_pubkey, &stealth_addr.view_tag);
        ...
        let derived_address =
            crate::derive_eth_address_from_private(&private_key.unwrap()).unwrap();
