...
pub struct Secp256k1PublicKey(
    #[serde_as(as = "serde_with::Bytes")] pub [u8; SECP256K1_PUBLIC_KEY_SIZE],
);
...
impl StealthAddress {
    ...
    pub fn decode(s: &str) -> NonosResult<Self> {
        let s = s
            .strip_prefix("st:")
            .ok_or_else(|| NonosError::InvalidAddress("Invalid stealth address prefix".into()))?;
...
impl WalletId {
    ...
    pub fn parse(s: &str) -> NonosResult<Self> {
...
mod tests {
    ...
    fn test_token_amount() {
        ...
        let sum = amount
            .checked_add(&TokenAmount::nox("0.5").unwrap())
            .unwrap();
