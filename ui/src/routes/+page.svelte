<script lang="ts">
	...
	function onError() {
		loading = false;
		error = 'Failed to load - check if Nym Mixnet is connected';
	}
...
<div class="page">
	{#if browsing}
		<div class="view">
			<iframe bind:this={frame} title="Browser" class="frame" srcdoc={content} sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-modals" on:load={onLoad} on:error={onError}></iframe>
			...
			<div class="badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg><span>Protected via Nym Mixnet</span></div>
		</div>
	{:else}
		<div class="home">
			<div class="content">
				...
				<p class="tagline">Private Browsing via Nym Mixnet</p>
				...
				<div class="grid">
					<button on:click={() => go('https://nymtech.net')}><div class="icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg></div><span>Nym Network</span></button>
					...
					<button on:click={() => go('https://shipyard.nymtech.net')}><div class="icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="5" r="3"/><circle cx="5" cy="19" r="3"/><circle cx="19" cy="19" r="3"/><path d="M12 8v4M8.5 14.5L5.5 17M15.5 14.5l3 2.5"/><circle cx="12" cy="14" r="2"/></svg></div><span>Nym Shipyard</span></button>
				...
				<p class="info"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>All traffic encrypted via Nym Mixnet</p>
			</div>
		</div>
	{/if}
</div>
...
.view{flex:1;display:flex;flex-direction:column;position:relative;background:#0a0a0f}
.frame{flex:1;width:100%;height:100%;border:none;background:#fff}
.bar{position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,transparent,rgba(102,255,255,0.1),transparent);overflow:hidden}
.progress{height:100%;width:40%;background:linear-gradient(90deg,transparent,#66FFFF,#00ff88,#66FFFF,transparent);animation:load 1.2s ease-in-out infinite;box-shadow:0 0 20px #66FFFF}
@keyframes load{0%{transform:translateX(-150%)}100%{transform:translateX(350%)}}
.err{position:absolute;top:16px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,rgba(255,50,100,0.95),rgba(180,30,70,0.95));color:#fff;padding:12px 24px;border-radius:12px;display:flex;align-items:center;gap:16px;font-size:14px;font-weight:500;box-shadow:0 8px 32px rgba(255,50,100,0.4);border:1px solid rgba(255,100,150,0.3);backdrop-filter:blur(12px)}
.err button{background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.2);color:#fff;padding:6px 16px;border-radius:6px;cursor:pointer;font-weight:500;transition:all 0.2s}
.err button:hover{background:rgba(255,255,255,0.25);transform:scale(1.05)}
.badge{position:absolute;bottom:16px;right:16px;display:flex;align-items:center;gap:8px;background:linear-gradient(135deg,rgba(10,16,21,0.95),rgba(0,20,30,0.95));padding:10px 18px;border-radius:24px;color:#00ff88;font-size:12px;font-weight:500;letter-spacing:0.5px;border:1px solid rgba(0,255,136,0.3);box-shadow:0 4px 24px rgba(0,255,136,0.2),inset 0 0 20px rgba(0,255,136,0.05);animation:badgePulse 3s ease-in-out infinite}
@keyframes badgePulse{0%,100%{box-shadow:0 4px 24px rgba(0,255,136,0.2),inset 0 0 20px rgba(0,255,136,0.05)}50%{box-shadow:0 4px 32px rgba(0,255,136,0.35),inset 0 0 30px rgba(0,255,136,0.1)}}
.badge svg{width:16px;height:16px;filter:drop-shadow(0 0 4px currentColor)}
.home{flex:1;display:flex;align-items:center;justify-content:center;padding:var(--nox-space-xl);overflow:auto;position:relative;background:radial-gradient(ellipse at 50% 0%,rgba(102,255,255,0.08) 0%,transparent 50%),radial-gradient(ellipse at 80% 80%,rgba(0,255,136,0.05) 0%,transparent 40%),radial-gradient(ellipse at 20% 80%,rgba(102,255,255,0.05) 0%,transparent 40%),linear-gradient(180deg,#0a0a12 0%,#0d0d1a 50%,#0a0a12 100%)}
.home::before{content:'';position:absolute;inset:0;background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(102,255,255,0.015) 2px,rgba(102,255,255,0.015) 4px);pointer-events:none;animation:scanlines 8s linear infinite}
@keyframes scanlines{0%{transform:translateY(0)}100%{transform:translateY(4px)}}
.home::after{content:'';position:absolute;inset:0;background:radial-gradient(circle at 50% 50%,transparent 0%,rgba(0,0,0,0.4) 100%);pointer-events:none}
.content{max-width:640px;width:100%;text-align:center;display:flex;flex-direction:column;align-items:center;position:relative;z-index:1}
.logo{width:300px;height:auto;filter:drop-shadow(0 0 80px rgba(102,255,255,0.6)) drop-shadow(0 0 40px rgba(102,255,255,0.4));margin-bottom:var(--nox-space-lg);animation:logoFloat 6s ease-in-out infinite}
@keyframes logoFloat{0%,100%{transform:translateY(0) scale(1);filter:drop-shadow(0 0 80px rgba(102,255,255,0.6)) drop-shadow(0 0 40px rgba(102,255,255,0.4))}50%{transform:translateY(-8px) scale(1.01);filter:drop-shadow(0 0 100px rgba(102,255,255,0.7)) drop-shadow(0 0 50px rgba(102,255,255,0.5))}}
.tagline{font-size:13px;color:rgba(102,255,255,0.7);letter-spacing:4px;text-transform:uppercase;margin-bottom:var(--nox-space-xl);font-weight:300}
.search{width:100%;margin-bottom:var(--nox-space-xl)}
.box{display:flex;align-items:center;background:linear-gradient(135deg,rgba(15,25,40,0.9),rgba(10,20,30,0.95));border:1px solid rgba(102,255,255,0.25);border-radius:16px;padding:6px;backdrop-filter:blur(20px);box-shadow:0 8px 40px rgba(0,0,0,0.4),inset 0 1px 0 rgba(102,255,255,0.1);position:relative;overflow:hidden}
.box::before{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(102,255,255,0.05),transparent);transform:translateX(-100%);transition:transform 0.6s}
.box:focus-within::before{transform:translateX(100%)}
.box:focus-within{border-color:#66FFFF;box-shadow:0 8px 40px rgba(0,0,0,0.4),0 0 0 4px rgba(102,255,255,0.15),0 0 60px rgba(102,255,255,0.15)}
.box svg{width:24px;height:24px;margin-left:var(--nox-space-md);color:rgba(102,255,255,0.5)}
.box input{flex:1;padding:var(--nox-space-md);background:transparent;border:none;font-size:17px;color:#fff;font-weight:300;letter-spacing:0.3px}
.box input:focus{outline:none}
.box input::placeholder{color:rgba(160,160,184,0.6)}
.box button{width:52px;height:52px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#66FFFF 0%,#00cc99 100%);border-radius:12px;color:#0a0a12;box-shadow:0 4px 20px rgba(102,255,255,0.3);position:relative;overflow:hidden}
.box button::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,0.3),transparent);opacity:0;transition:opacity 0.2s}
.box button:hover::before{opacity:1}
.box button svg{width:22px;height:22px;margin:0;position:relative;z-index:1}
.box button:hover{box-shadow:0 6px 30px rgba(102,255,255,0.5);transform:scale(1.05)}
.box button:active{transform:scale(0.98)}
.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;width:100%;margin-bottom:var(--nox-space-xl)}
.grid button{display:flex;flex-direction:column;align-items:center;gap:10px;padding:20px 16px;background:linear-gradient(145deg,rgba(18,28,40,0.8),rgba(12,20,30,0.9));border:1px solid rgba(102,255,255,0.12);border-radius:14px;backdrop-filter:blur(10px);color:rgba(200,200,220,0.9);font-size:12px;font-weight:500;letter-spacing:0.3px;position:relative;overflow:hidden;transition:all 0.3s cubic-bezier(0.4,0,0.2,1)}
.grid button::before{content:'';position:absolute;inset:0;background:radial-gradient(circle at 50% 0%,rgba(102,255,255,0.15),transparent 70%);opacity:0;transition:opacity 0.3s}
.grid button:hover::before{opacity:1}
.grid button::after{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:linear-gradient(45deg,transparent,rgba(102,255,255,0.03),transparent);transform:rotate(45deg) translateY(-100%);transition:transform 0.6s}
.grid button:hover::after{transform:rotate(45deg) translateY(100%)}
.grid button:hover{border-color:rgba(102,255,255,0.5);background:linear-gradient(145deg,rgba(25,40,55,0.9),rgba(15,25,40,0.95));transform:translateY(-4px);box-shadow:0 12px 40px rgba(0,0,0,0.3),0 0 30px rgba(102,255,255,0.1)}
.grid button span{position:relative;z-index:1}
.icon{width:48px;height:48px;display:flex;align-items:center;justify-content:center;border-radius:12px;background:linear-gradient(135deg,rgba(102,255,255,0.15),rgba(102,255,255,0.05));color:#66FFFF;position:relative;z-index:1;transition:all 0.3s}
.grid button:hover .icon{background:linear-gradient(135deg,rgba(102,255,255,0.25),rgba(102,255,255,0.1));box-shadow:0 0 20px rgba(102,255,255,0.2);transform:scale(1.1)}
.icon svg{width:24px;height:24px}
.icon.nox svg{width:30px;height:30px}
.info{display:flex;align-items:center;justify-content:center;gap:10px;font-size:12px;color:rgba(102,255,255,0.5);font-weight:400;letter-spacing:1px;padding:12px 24px;background:linear-gradient(135deg,rgba(102,255,255,0.05),transparent);border-radius:30px;border:1px solid rgba(102,255,255,0.1)}
.info svg{width:18px;height:18px;color:#00ff88;filter:drop-shadow(0 0 4px currentColor)}
@media(max-width:600px){.grid{grid-template-columns:repeat(2,1fr)}.logo{width:220px}.tagline{font-size:11px;letter-spacing:3px}}
</style>
