use super::binary::find_nonos_node_binary;
...
pub async fn node_start_embedded(state: State<'_, AppState>, window: Window) -> Result<(), String> {
            ...
            for line in reader.lines().map_while(Result::ok) {
                if line.contains("Node ID:") {
                    if let Some(id) = line.split("Node ID:").nth(1) {
                        let _ = window_clone.emit("nonos://node-id", id.trim());
                    }
                }
                if line.contains("NONOS node started successfully")
    ...
    window.emit("nonos://node-started", serde_json::json!({ "pid": pid, "api_addr": "http://127.0.0.1:8080", "p2p_port": 9432 })).map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
pub async fn node_stop_embedded(state: State<'_, AppState>, window: Window) -> Result<(), String> {
    let mut nodes = state.nodes.write().await;
