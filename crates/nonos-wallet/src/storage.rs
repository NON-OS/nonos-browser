...
impl FileWalletStorage {
    pub fn new(base_dir: impl AsRef<Path>) -> NonosResult<Self> {
        ...
        if !base_dir.exists() {
            std::fs::create_dir_all(&base_dir).map_err(|e| NonosError::Storage(e.to_string()))?;
        }
    ...
    fn save_wallet_file(&self, wallet: &WalletFile) -> NonosResult<()> {
        let id = WalletId::parse(&wallet.id)?;
        let path = self.wallet_path(&id);
...
impl WalletStorage for FileWalletStorage {
    fn list_wallets(&self) -> NonosResult<Vec<WalletId>> {
        ...
        let entries =
            std::fs::read_dir(&self.base_dir).map_err(|e| NonosError::Storage(e.to_string()))?;
        ...
        for entry in entries {
            ...
            if path.extension().is_some_and(|ext| ext == "wallet") {
                if let Some(stem) = path.file_stem() {
                    if let Ok(id) = WalletId::parse(&stem.to_string_lossy()) {
                        wallets.push(id);
                    }
                }
            }
    ...
    fn save_wallet(
        ...
        let plaintext =
            serde_json::to_vec(&secrets).map_err(|e| NonosError::Serialization(e.to_string()))?;
...
impl WalletStorage for MemoryWalletStorage {
    fn list_wallets(&self) -> NonosResult<Vec<WalletId>> {
        ...
        wallets.keys().map(|id| WalletId::parse(id)).collect()
    }
...
mod tests {
    ...
    fn test_memory_storage() {
        ...
        storage
            .save_wallet(&metadata, &master_key, password)
            .unwrap();
    ...
    fn test_password_change() {
        ...
        storage
            .save_wallet(&metadata, &master_key, old_password)
            .unwrap();
        storage
            .change_password(&metadata.id, old_password, new_password)
            .unwrap();
